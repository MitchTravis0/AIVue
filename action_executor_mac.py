...
def take_screenshot(filename=None, region=None):
    """
    Takes a screenshot of the current screen and saves it to the screenshot directory.
    ...
    try:
        ...
        # Capture the entire screen (or a region if specified)
        if region:
...
def move_mouse_to(x, y, duration=0.5):
    ...
    try:
        ...
        for step in range(steps + 1):
            intermediate_x = start_x + (x - start_x) * step / steps
            intermediate_y = start_y + (y - start_y) * step / steps
            mouse.position = (intermediate_x, intermediate_y)
            time.sleep(duration / steps)  # Control the speed of movement
...
def drag_mouse(start_x, start_y, end_x, end_y, duration=0.5):
    """
    Clicks and drags the mouse from one position to another using pynput.
    ...
    try:
        ...
        # Press and hold the left mouse button, then drag
        mouse.press(Button.left)
        # Calculate the total number of steps based on duration
        steps = int(duration * 100)

        for i in range(steps):
            # Interpolate between the start and end positions over the duration
            x = int(start_x + (end_x - start_x) * (i / steps))
            y = int(start_y + (end_y - start_y) * (i / steps))
            mouse.position = (x, y)
            time.sleep(duration / steps)

        # Release the left mouse button to complete the drag
...
def scroll(direction, clicks=3):
    """
    Scrolls the page in specified direction using pynput.
    ...
    try:
        ...
        if direction.lower() == 'down':
            ...
            mouse.scroll(0, -amount)  # Negative value scrolls down
        elif direction.lower() == 'up':
            print(f"ACTION: Scrolling up {amount} clicks")
            mouse.scroll(0, amount)  # Positive value scrolls up
        else:
    ...
    if use_json_format:
        ...
        try:
            ...
            if command_name == "run_app":
                ...
                print(f"Running {app} via spotlight search...")
        ...
        try:
            ...
            if command_name == "run_app":
                # This is now deprecated - we should warn the user
                print(
                    f"WARNING: Direct app launching with 'run_app' is deprecated. The agent should use vision-guided actions to find and open {args_part}.")
        ...
        if action_obj.get("action") == "find_element" and ELEMENT_DETECTOR_AVAILABLE:
            ...
            if description:
                ...
                if element and get_element_coordinates(element):
                    ...
                    if i < len(actions) and actions[i].get("action") in ["move_mouse", "click", "right_click",
                                                                         "double_click"]:
