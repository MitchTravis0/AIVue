...
class HeadGazeTracker:
    ...
    def run_calibration(self, cap):
        ...
        feed_w = self.screen_width // 8  # Smaller width for the feed
        feed_h = self.screen_height // 8  # Smaller height for the feed
        ...
        center_x, center_y = actual_w // 2 + 60, actual_h // 2
        cv2.circle(calib_img, (center_x, center_y), 30, (238, 0, 255), 6)
        cv2.circle(calib_img, (center_x, center_y), 15, (255, 0, 0), -1)
        cv2.putText(calib_img, "Look Here (Neutral)", (center_x - 100, center_y - 60),
        ...
        while time.time() - start_time < collection_duration:
            ...
            progress_fraction = min(
                1.0, (time.time() - start_time) / collection_duration)
            progress_bar_width = int(actual_w * progress_fraction)
            cv2.rectangle(progress_img, (0, actual_h - 20),
                          (progress_bar_width, actual_h), (0, 255, 0), -1)
        ...
        if len(neutral_data_raw) >= min_neutral_samples:
            ...
            print(
                f"Neutral calibrated: Raw Yaw={avg_yaw:.3f}, Raw Pitch={avg_pitch:.3f}")
        else:
            print(
                f"Failed neutral samples ({len(neutral_data_raw)}/{min_neutral_samples}). Try again.")
            cv2.destroyAllWindows()
            return False
        ...
        for i, (x_ratio, y_ratio) in enumerate(self.calibration_points):
            ...
            print(
                f"Point {i+1}/{len(self.calibration_points)}: Look at target ({x_ratio:.1f}, {y_ratio:.1f})...")
            point_data_raw = []
            baseline_pose = None
            start_time = time.time()
            self.first_pose = True
            while time.time() - start_time < collection_duration_point:
                ...
                # point_img = np.full((actual_h, actual_w, 3),
                #                     background_color, dtype=np.uint8)
                cv2.circle(point_img, (target_x, target_y),
                           30, (238, 0, 255), 6)
                cv2.circle(point_img, (target_x, target_y), 15, (255, 0, 0), -1)
                cv2.putText(point_img, f"Look Here ({i+1})",
                            (target_x - 80 if target_x > 50 else 5,
                             target_y - 40 if target_y > 40 else 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
                remaining_time = collection_duration_point - \
                    (time.time() - start_time)
                cv2.putText(point_img, f"{remaining_time:.1f}s",
                (target_x - 20 if target_x > 20 else 5, target_y +
                55 if target_y < actual_h - 65 else actual_h - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                progress_fraction = min(
                    1.0, (time.time() - start_time) / collection_duration_point)
                bar_width = 100
                bar_height = 10
                bar_x = target_x - bar_width // 2
                bar_y = target_y + 80
                filled_width = int(bar_width * progress_fraction)
                cv2.rectangle(point_img, (bar_x, bar_y), (bar_x +
                              filled_width, bar_y + bar_height), (0, 255, 0), -1)
                cv2.rectangle(point_img, (bar_x, bar_y), (bar_x +
                              bar_width, bar_y + bar_height), (255, 255, 255), 1)
                cv2.putText(point_img, "Stay still", (bar_x + 12, bar_y - 8),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            ...
            if len(point_data_raw) >= min_point_samples:
                ...
                self.calibration_data.append(
                    (avg_yaw, avg_pitch, x_ratio, y_ratio))
                print(
                    f"  Point {i+1} recorded (RawYaw={avg_yaw:.3f}, RawPitch={avg_pitch:.3f}) -> Target ({x_ratio:.1f}, {y_ratio:.1f})")
            else:
                print(
                    f"  Skipped point {i+1}: Insufficient samples ({len(point_data_raw)}/{min_point_samples}).")
        ...
        if len(self.calibration_data) >= 6:
            ...
            print(
                f"Calibration failed: Not enough points ({len(self.calibration_data)}). Need 6+.")
            self.calibration_complete = False
            return False
    ...
    def map_head_to_screen(self, head_yaw, head_pitch):
        ...
        features = np.array([rel_yaw, rel_pitch, rel_yaw **
                            2, rel_pitch**2, rel_yaw * rel_pitch, 1.0])
