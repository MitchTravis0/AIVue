...
class AssistWidget(QDialog):
    ...
    def __init__(self):
        ...
        self.input.setStyleSheet("font-size: 32px")

        self.send_button = QPushButton("Send")
        self.send_button.setStyleSheet("font-size: 32px")
        self.send_button.clicked.connect(self.handle_send)

        layout.addWidget(QLabel("Enter Msg"))
    ...
    def handle_send(self):
        ...
        self.prompt_result = self.input.text()
        message_output = (f"{message}")
        print(f"Sending message: {message}")

        # pause cam

        # digest prompts thorugh the execution

        action_exectuor_mac.digest_prompts(message_output)
...
class ProcessControlApp(QWidget):
    ...
    def show_assist_ui(self):
        ...
        if assist_popup.exec():
            prompt_result = assist_popup.prompt_result
